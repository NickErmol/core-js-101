...
function isInsideCircle(circle, point) {
  const rad = Math.hypot(point.x - circle.center.x, point.y - circle.center.y);
...
function findFirstSingleChar(str) {
  let result = null;
  str.split('').filter((elem, index, arr) => {
    if ((arr.indexOf(elem) === arr.lastIndexOf(elem)) && (result === null)) {
      result = elem;
    }
    return result;
  });
...
function isCreditCardNumber(ccn) {
  const intArr = `${ccn}`.split('').reverse().map((x) => Number(x));
  const result = intArr.map((elem, index) => {
    let digit;
    if (index % 2 !== 0) {
      digit = elem * 2;
    } else {
...
function getDigitalRoot(num) {
  ...
  const result = intArr.reduce((elem, acc) => elem + acc);
...
function evaluateTicTacToePosition(position) {
  if (position[0][0] === position[1][1]
    && position[1][1] === position[2][2] && position[0][0] !== '') return position[0][0];
